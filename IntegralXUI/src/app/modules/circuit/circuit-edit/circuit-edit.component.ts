import { CommonModule } from '@angular/common';
import { Component, OnInit } from '@angular/core';
import { FormBuilder, FormGroup, ReactiveFormsModule, Validators } from '@angular/forms';
import { Router } from '@angular/router';
import { AuthService } from 'src/app/services/auth/auth.service';
import { CircuitService } from 'src/app/services/circuit/circuit.service';
import Swal from 'sweetalert2';
import { CircuitDocs } from '../circuit';
import { SharedDataService } from 'src/app/services/shared-data/shared-data.service';
import { LoadingService } from 'src/app/common/loadingPanel/loading.service';

@Component({
  selector: 'app-circuit-edit',
  imports: [ReactiveFormsModule, CommonModule],
  templateUrl: './circuit-edit.component.html',
  styleUrl: './circuit-edit.component.scss'
})
export class CircuitEditComponent {

 receivedData: any;
    allExpanded = false;
    clientData: any;
    childValue: number = 0;
    documents: CircuitDocs[] = [];
    canEdit: boolean = false;
    canDelete: boolean = false;
    documentsToDelete: number[] = [];

  circuitForm!: FormGroup;
  documentPreviews: File[] = [];
  canAdd: boolean = false;
  circuitTypes: any[] = [];
  circuitStatus: any[] = [];
  plants: any[] = [];
  areas: any[] = [];
  units: any[] = [];
  systems: any[] = [];
  designCodeList: any[] = [];
  editionList: any[] = [];
  complianceCertList: any[] = [];
  geometryList: any[] = [];
  orientationList: any[] = [];
  fluidPhaseList: any[] = [];
  corrosivityList: any[] = [];
  environmentList: any[] = [];
  toxicMixtureList: any[] = [];
  toxicFluidList: any[] = [];
  flammabilityList: any[] = [];
  criticalityList: any[] = [];
  seismicZoneList: any[] = [];
  fireExplosionRiskList: any[] = [];
  toxicRiskList: any[] = [];

  heatTreatmentList: any[] = [];
  heatTreatmentTypeList: any[] = [];
  impactTestList: any[] = [];
  pressureTestList: any[] = [];
  pressureTestTypeList: any[] = [];
  radiographyList: any[] = [];
  radiographyCategoryList: any[] = [];

  generalMaterialList: any[] = [];
  scheduleList: any[] = [];
  claddingList: any[] = [];
  claddingTypeList: any[] = [];
  claddingMaterialList: any[] = [];
  liningList: any[] = [];
  materialCertificationList: any[] = [];
  liningTypeList: any[] = [];

  externalCoatingList: any[] = [];
  externalCoatingTypeList: any[] = [];
  insulationList: any[] = [];
  insulationTypeList: any[] = [];
  insulationMaterialList: any[] = [];
  cuiPotentialList: any[] = [];
  externalEnvironmentList: any[] = [];
  supportTypeList: any[] = [];
  heatTracingList: any[] = [];
  fireProofingList: any[] = [];
  buriedList: any[] = [];
  cathodicProtectionList: any[] = [];

  pressureReliefDevicesList: any[] = [];
  chemicalInjectionList: any[] = [];
  detectionSystemList: any[] = [];
  isolationSystemList: any[] = [];
  mitigationSystemList: any[] = [];
  onlineCorrosionMonitoringList: any[] = [];
  corrosionMonitoringTypeList: any[] = [];
  hazardClassificationList: any[] = [];

  inspectionAccessList: any[] = [];
  inspectionStrategyList: any[] = [];
  scheduledRepairList: any[] = [];
  expand = true
  showGeneral = this.expand;
  showLocation = this.expand;
  showDesign = this.expand;
  showGeometry = this.expand;
  showOperation = this.expand;
  showFabrication = this.expand;
  showMaterial = this.expand;
  showExternal = this.expand;
  showSafety = this.expand;
  showIntegrity = this.expand;
  showInspectionSchedule = this.expand;
  showMaintenance = this.expand;
  showMetadata = this.expand;
  showAudit = this.expand;
  showDocument = this.expand;

  constructor(private circuitService: CircuitService, private fb: FormBuilder,private ls: LoadingService,  private au: AuthService,private sharedDataService: SharedDataService, private router: Router) {
    this.canDelete = (this.au.getCanDelete());
    this.canEdit = (this.au.getCanEdit());
    this.receivedData = this.sharedDataService.getData();
    this.childValue = this.receivedData.id;
   
    this.circuitForm = this.fb.group({
      id: [0],
      clientId: [this.au.getClientId()],
      name: [''],
      circuitDescription: [''],
      circuitTypeId: [null],
      commissioningDate: [null],
      builtDate: [null],
      operationalStatusId: [null],
      plantID: [null, Validators.required],
      areaID: [null],
      unitID: [null],
      systemID: [null],
      corrosionLoopID: [null],
      specificLocation: [''],
      circuitFrom: [''],
      circuitTo: [''],
      pfd: [''],
      pnID: [''],
      isometricDrawing: [''],
      designCodeId: [null],
      editionAndAddendumId: [null],
      designPressureInternal: [''],
      designPressureExternal: [''],
      designTemperatureMax: [''],
      designTemperatureMDMT: [''],
      mawp: [''],
      designLife: [''],
      primaryProducts: [''],
      marginPerDay: [''],
      complianceCertificationId: [null],
      governingRegulatoryBody: [''],
      geometryId: [null],
      nominalSizeNPS: [null],
      insideDiameter: [null],
      outsideDiameter: [null],
      lengthOrHeight: [null],
      fillHeight: [null],
      orientationId: [null],
      emptyWeight: [null],
      capacity: [null],
      operatingPressureMax: [null],
      operatingPressureAvg: [null],
      operatingTempMax: [null],
      operatingTempMin: [null],
      processFluid: [''],
      fluidComposition: [''],
      fluidPhaseId: [null],
      inventoryVolume: [null],
      density: [null],
      viscosity: [null],
      specificGravity: [null],
      flowRate: [null],
      velocity: [null],
      corrosivityId: [null],
      ph: [null],
      processEnvironmentId: [null],
      toxicMixtureId: [null],
      toxicFluidId: [null],
      flammabilityId: [null],
      operatingWeight: [null],
      criticalityId: [null],
      humidityLevel: [null],
      seismicZoneClassificationId: [null],
      fireAndExplosionRiskId: [null],
      toxicRiskId: [null],
      totalPopulation: [null],
      populationDensity: [null],
      manufacturer: [''],
      serialNumber: [''],
      warrantyDate: [null],
      heatTreatmentId: [null],
      heatTreatmentTypeId: [null],
      impactTestId: [null],
      pressureTestId: [null],
      pressureTestTypeId: [null],
      testPressure: [''],
      radiographyId: [null],
      radiographyCategoryid: [null],
      jointEfficiency: [''],
      otherNDE: [''],
      generalMaterialId: [null],
      materialSpecification: [''],
      allowableStress: [null],
      scheduleid: [null],
      nominalThickness: [null],
      corrosionAllowance: [null],
      claddingId: [null],
      claddingTypeId: [null],
      claddingMaterialId: [null],
      claddingThickness: [null],
      liningId: [null],
      liningTypeId: [null],
      materialCertificationId: [null],
      externalCoatingId: [null],
      externalCoatingTypeId: [null],
      externalCoatingThickness: [null],
      externalCoatingAge: [null],
      insulationId: [null],
      insulationTypeId: [null],
      insulationMaterialId: [null],
      insulationThickness: [''],
      cuiPotentialId: [null],
      externalEnvironmentId: [null],
      supportTypeId: [null],
      heatTracingId: [null],
      fireProofingId: [null],
      buriedId: [null],
      cathodicProtectionId: [null],
      pressureReliefDevicesId: [null],
      prdID: [''],
      prdSetPressure: [null],
      chemicalInjectionId: [null],
      detectionSystemId: [null],
      isolationSystemId: [null],
      mitigationSystemId: [null],
      onlineCorrosionMonitoringId: [null],
      corrosionMonitoringTypeId: [null],
      hazardClassificationId: [null],
      safetyAndEnvironmentalPermitsId: [''],
      incidentHistory: [''],
      currentInspectionStrategy: [null],
      damageMechanisms: [''],
      shutdownFrequency: [null],
      lastMajorShutdownDate: [null],
      nextMajorShutdownDate: [null],
      subsequentMajorShutdownDate: [null],
      mtbf: [null],
      mttr: [null],
      cmlDrawingID: [''],
      inspectionAccess: [null],
      inspectionSupervisor: [''],
      inspector: [''],
      inspectionTypeId: [null],
      internalInspectionDate: [null],
      externalInspectionDate: [null],
      onStreamInspectionDate: [null],
      tmInspectionDate: [null],
      scheduledRepairOrReplacementId: [null],
      scheduledRepairOrReplacementDate: [null],
      repairDuringNextShutdown: [false],
      erpCircuitCode: [''],
      erpSystem: [''],
      functionalLocation: [''],
      externalSystemID: [''],
      syncStatus: [''],
      createdBy: [null],
      createdDate: [null],
      lastModifiedBy: [null],
      lastModifiedDate: [null],
      isDeleted: [false],
      isActive: [true],
      deletedFiles: [''],
      createdUser: [''],
      modifiedUser: [''],
      plant: [''],
      area: [''],
      unit: [''],
      system: [''],
      status: [''],
      type: [''],
      userId: [null]
    });
  }
  toggle(section: string) {
    this[section] = !this[section];
  }
  // Expand all sections
  expandAll() {
    this.setAll(true);
  }

  // Collapse all sections
  collapseAll() {
    this.setAll(false);
  }
  async ngOnInit() {
    this.ls.showLoading();
    await this.loadDropdowns();
    
   setTimeout(async () => {
    await this.loadCircuitDetails(); // sets selected value
  }, 10);
   await this.circuitForm.get('plantID')?.valueChanges.subscribe((plantID) => {
     
      this.circuitForm.get('name')?.updateValueAndValidity();
      if (plantID) {
        this.loadAreasByPlant(plantID);
        this.loadUnits(plantID, this.circuitForm.get('areaID').value??0);
        this.loadSystems(plantID, this.circuitForm.get('areaID').value??0, this.circuitForm.get('unitID').value??0);
        this.ls.hideLoading();
      } else {
        this.areas = [];
        this.units = [];
        this.systems = []; // Reset area list if no plant is selected
      }
    });
   await this.circuitForm.get('areaID')?.valueChanges.subscribe((areaID) => {

      this.circuitForm.get('name')?.updateValueAndValidity();
      if (areaID) {

        this.loadUnits(this.circuitForm.get('plantID').value, areaID);
        this.loadSystems(this.circuitForm.get('plantID').value, this.circuitForm.get('areaID').value??0, this.circuitForm.get('unitID').value??0);

      } else {

        this.units = [];
        this.systems = []; // Reset area list if no plant is selected
      }
    });
    this.circuitForm.get('unitID')?.valueChanges.subscribe((unitID) => {

      this.circuitForm.get('name')?.updateValueAndValidity();
      if (unitID) {

        this.loadSystems(this.circuitForm.get('plantID').value, this.circuitForm.get('areaID').value, this.circuitForm.get('unitID').value??0);

      } else {

        this.systems = []; // Reset area list if no plant is selected
      }
    });

    this.circuitForm.get('areaID')?.valueChanges.subscribe(() => {
      this.circuitForm.get('name')?.updateValueAndValidity();
    });
    this.circuitForm.get('unitID')?.valueChanges.subscribe(() => {
      this.circuitForm.get('name')?.updateValueAndValidity();
    });
    this.circuitForm.get('unitID')?.valueChanges.subscribe(() => {
      this.circuitForm.get('name')?.updateValueAndValidity();
    });
    this.circuitForm.get('systemId')?.valueChanges.subscribe(() => {
      this.circuitForm.get('name')?.updateValueAndValidity();
    });
  
  }
    saveCircuit() {
    
      if (this.circuitForm.invalid) {
        alert();
        return;
      }
      const formData = new FormData();
      Object.keys(this.circuitForm.value).forEach(key => {
        const value = this.circuitForm.value[key];
  
        // Only append values that are not empty strings or "null"
        if (value !== "" && value !== "null" && value !== null && value !== undefined) {
          formData.append(key, value);
        }
      });
      formData.append('deletedFiles', JSON.stringify(this.documentsToDelete).toString());
    
      this.documentPreviews.forEach(file => {
        formData.append('documents', file);
      });
  
      this.circuitService.addCircuit(formData).subscribe(
        (response) => {
  
  
          Swal.fire({
            title: 'Success!',
            text: 'Circuit updated successfully',
            icon: 'success',
            confirmButtonText: 'Ok'
          });
          this.backToCircuit();
        }
      );
    }
    backToCircuit() {
      this.router.navigate(['/clientcircuit/list']);
    }
    deleteFileExisting(documentId: number) {
      this.documentsToDelete.push(documentId);
      this.documents = this.documents.filter(doc => doc.id !== documentId); // Remove from UI
    }
  
  loadCircuitDetails() {
    this.circuitService.getCircuitDetails(this.childValue).subscribe((circuitData: any) => {
      debugger;
      this.circuitForm.patchValue({
        id: circuitData.id,
        clientId: circuitData.clientId,
        name: circuitData.name,
        circuitDescription: circuitData.circuitDescription,
        circuitTypeId: circuitData.circuitTypeId,
        commissioningDate: circuitData.commissioningDate ? new Date(circuitData.commissioningDate).toLocaleDateString('en-CA') : null,
        builtDate: circuitData.builtDate ? new Date(circuitData.builtDate).toLocaleDateString('en-CA') : null,
        operationalStatusId: circuitData.operationalStatusId,
        plantID: circuitData.plantID,
        areaID: circuitData.areaID,
        unitID: circuitData.unitID,
        systemID: circuitData.systemID,
        corrosionLoopID: circuitData.corrosionLoopID,
        specificLocation: circuitData.specificLocation,
        circuitFrom: circuitData.circuitFrom,
        circuitTo: circuitData.circuitTo,
        pfd: circuitData.pfd,
        pnID: circuitData.pnID,
        isometricDrawing: circuitData.isometricDrawing,
        designCodeId: circuitData.designCodeId,
        editionAndAddendumId: circuitData.editionAndAddendumId,
        designPressureInternal: circuitData.designPressureInternal,
        designPressureExternal: circuitData.designPressureExternal,
        designTemperatureMax: circuitData.designTemperatureMax,
        designTemperatureMDMT: circuitData.designTemperatureMDMT,
        mawp: circuitData.mawp,
        designLife: circuitData.designLife,
        primaryProducts: circuitData.primaryProducts,
        marginPerDay: circuitData.marginPerDay,
        complianceCertificationId: circuitData.complianceCertificationId,
        governingRegulatoryBody: circuitData.governingRegulatoryBody,
        geometryId: circuitData.geometryId,
        nominalSizeNPS: circuitData.nominalSizeNPS,
        insideDiameter: circuitData.insideDiameter,
        outsideDiameter: circuitData.outsideDiameter,
        lengthOrHeight: circuitData.lengthOrHeight,
        fillHeight: circuitData.fillHeight,
        orientationId: circuitData.orientationId,
        emptyWeight: circuitData.emptyWeight,
        capacity: circuitData.capacity,
        operatingPressureMax: circuitData.operatingPressureMax,
        operatingPressureAvg: circuitData.operatingPressureAvg,
        operatingTempMax: circuitData.operatingTempMax,
        operatingTempMin: circuitData.operatingTempMin,
        processFluid: circuitData.processFluid,
        fluidComposition: circuitData.fluidComposition,
        fluidPhaseId: circuitData.fluidPhaseId,
        inventoryVolume: circuitData.inventoryVolume,
        density: circuitData.density,
        viscosity: circuitData.viscosity,
        specificGravity: circuitData.specificGravity,
        flowRate: circuitData.flowRate,
        velocity: circuitData.velocity,
        corrosivityId: circuitData.corrosivityId,
        ph: circuitData.ph,
        processEnvironmentId: circuitData.processEnvironmentId,
        toxicMixtureId: circuitData.toxicMixtureId,
        toxicFluidId: circuitData.toxicFluidId,
        flammabilityId: circuitData.flammabilityId,
        operatingWeight: circuitData.operatingWeight,
        criticalityId: circuitData.criticalityId,
        humidityLevel: circuitData.humidityLevel,
        seismicZoneClassificationId: circuitData.seismicZoneClassificationId,
        fireAndExplosionRiskId: circuitData.fireAndExplosionRiskId,
        toxicRiskId: circuitData.toxicRiskId,
        totalPopulation: circuitData.totalPopulation,
        populationDensity: circuitData.populationDensity,
        manufacturer: circuitData.manufacturer,
        serialNumber: circuitData.serialNumber,
        warrantyDate: circuitData.warrantyDate ? new Date(circuitData.warrantyDate).toLocaleDateString('en-CA') : null,
        heatTreatmentId: circuitData.heatTreatmentId,
        heatTreatmentTypeId: circuitData.heatTreatmentTypeId,
        impactTestId: circuitData.impactTestId,
        pressureTestId: circuitData.pressureTestId,
        pressureTestTypeId: circuitData.pressureTestTypeId,
        testPressure: circuitData.testPressure,
        radiographyId: circuitData.radiographyId,
        radiographyCategoryid: circuitData.radiographyCategoryid,
        jointEfficiency: circuitData.jointEfficiency,
        otherNDE: circuitData.otherNDE,
        generalMaterialId: circuitData.generalMaterialId,
        materialSpecification: circuitData.materialSpecification,
        allowableStress: circuitData.allowableStress,
        scheduleid: circuitData.scheduleid,
        nominalThickness: circuitData.nominalThickness,
        corrosionAllowance: circuitData.corrosionAllowance,
        claddingId: circuitData.claddingId,
        claddingTypeId: circuitData.claddingTypeId,
        claddingMaterialId: circuitData.claddingMaterialId,
        claddingThickness: circuitData.claddingThickness,
        liningId: circuitData.liningId,
        liningTypeId: circuitData.liningTypeId,
        materialCertificationId: circuitData.materialCertificationId,
        externalCoatingId: circuitData.externalCoatingId,
        externalCoatingTypeId: circuitData.externalCoatingTypeId,
        externalCoatingThickness: circuitData.externalCoatingThickness,
        externalCoatingAge: circuitData.externalCoatingAge,
        insulationId: circuitData.insulationId,
        insulationTypeId: circuitData.insulationTypeId,
        insulationMaterialId: circuitData.insulationMaterialId,
        insulationThickness: circuitData.insulationThickness,
        cuiPotentialId: circuitData.cuiPotentialId,
        externalEnvironmentId: circuitData.externalEnvironmentId,
        supportTypeId: circuitData.supportTypeId,
        heatTracingId: circuitData.heatTracingId,
        fireProofingId: circuitData.fireProofingId,
        buriedId: circuitData.buriedId,
        cathodicProtectionId: circuitData.cathodicProtectionId,
        pressureReliefDevicesId: circuitData.pressureReliefDevicesId,
        prdID: circuitData.prdID,
        prdSetPressure: circuitData.prdSetPressure,
        chemicalInjectionId: circuitData.chemicalInjectionId,
        detectionSystemId: circuitData.detectionSystemId,
        isolationSystemId: circuitData.isolationSystemId,
        mitigationSystemId: circuitData.mitigationSystemId,
        onlineCorrosionMonitoringId: circuitData.onlineCorrosionMonitoringId,
        corrosionMonitoringTypeId: circuitData.corrosionMonitoringTypeId,
        hazardClassificationId: circuitData.hazardClassificationId,
        safetyAndEnvironmentalPermitsId: circuitData.safetyAndEnvironmentalPermitsId,
        incidentHistory: circuitData.incidentHistory,
        currentInspectionStrategy: circuitData.currentInspectionStrategy,
        damageMechanisms: circuitData.damageMechanisms,
        shutdownFrequency: circuitData.shutdownFrequency,
        lastMajorShutdownDate: circuitData.lastMajorShutdownDate ? new Date(circuitData.lastMajorShutdownDate).toLocaleDateString('en-CA') : null,
        nextMajorShutdownDate: circuitData.nextMajorShutdownDate ? new Date(circuitData.nextMajorShutdownDate).toLocaleDateString('en-CA') : null,
        subsequentMajorShutdownDate: circuitData.subsequentMajorShutdownDate ? new Date(circuitData.subsequentMajorShutdownDate).toLocaleDateString('en-CA') : null,
        mtbf: circuitData.mtbf,
        mttr: circuitData.mttr,
        cmlDrawingID: circuitData.cmlDrawingID,
        inspectionAccess: circuitData.inspectionAccess,
        inspectionSupervisor: circuitData.inspectionSupervisor,
        inspector: circuitData.inspector,
        inspectionTypeId: circuitData.inspectionTypeId,
        internalInspectionDate: circuitData.internalInspectionDate ? new Date(circuitData.internalInspectionDate).toLocaleDateString('en-CA') : null,
        externalInspectionDate: circuitData.externalInspectionDate ? new Date(circuitData.externalInspectionDate).toLocaleDateString('en-CA') : null,
        onStreamInspectionDate: circuitData.onStreamInspectionDate ? new Date(circuitData.onStreamInspectionDate).toLocaleDateString('en-CA') : null,
        tmInspectionDate: circuitData.tmInspectionDate ? new Date(circuitData.tmInspectionDate).toLocaleDateString('en-CA') : null,
        scheduledRepairOrReplacementId: circuitData.scheduledRepairOrReplacementId,
        scheduledRepairOrReplacementDate: circuitData.scheduledRepairOrReplacementDate ? new Date(circuitData.scheduledRepairOrReplacementDate).toLocaleDateString('en-CA') : null,
        repairDuringNextShutdown: circuitData.repairDuringNextShutdown,
        erpCircuitCode: circuitData.erpCircuitCode,
        erpSystem: circuitData.erpSystem,
        functionalLocation: circuitData.functionalLocation,
        externalSystemID: circuitData.externalSystemID,
        syncStatus: circuitData.syncStatus,
        modifiedBy:[this.au.getUserId()],
        userId:[this.au.getUserId()],
        createdBy: circuitData.createdBy,
        createdDate: circuitData.createdDate,
        lastModifiedBy: circuitData.lastModifiedBy,
        lastModifiedDate: circuitData.lastModifiedDate,
        isDeleted: circuitData.isDeleted,
        isActive: circuitData.isActive,
        deletedFiles: circuitData.deletedFiles,
        createdUser: circuitData.createdUser,
        modifiedUser: circuitData.modifiedUser,
        plant: circuitData.plant,
        area: circuitData.area,
        unit: circuitData.unit,
        system: circuitData.system,
        status: circuitData.status,
        type: circuitData.type
      });
      this.documents = (circuitData.docs);
    });
  }
  
  onPlantChange() {

    this.circuitForm.get('plantID')?.valueChanges.subscribe(plantID => {
      if (plantID) {
        this.loadAreasByPlant(plantID);

      } else {
        this.areas = []; // Reset area list if no plant is selected
      }
    });
  }
  loadAreasByPlant(plantID: number) {

    this.circuitService.getArea(plantID).subscribe((data: any[]) => {
      this.areas = data;
    });
  }
  loadUnits(plantID: number, areaID: number) {
debugger;
    this.circuitService.getUnits(plantID, areaID).subscribe((data: any[]) => {
      this.units = data;
    });
  }
  loadSystems(plantID: number, areaID: number, unitID: number) {

    this.circuitService.getSystems(plantID, areaID, unitID).subscribe((data: any[]) => {
      this.systems = data;
    });
  }


  loadDropdowns() {


    // Load dropdown data from services
    this.circuitService.getPlants().subscribe((data: any) => {
      debugger;
      this.plants = data;
    });
    this.circuitService.getCircuitTypes().subscribe((data: any) => {
      this.circuitTypes = data;
    });
    this.circuitService.getOperationalStatus().subscribe((data: any) => {
      this.circuitStatus = data;
    });

    this.circuitService.getDesignCode().subscribe((data: any) => {
      this.designCodeList = data;
    });
    this.circuitService.getEditionAndAddendum().subscribe((data: any) => {
      this.editionList = data;
    });
    this.circuitService.getComplianceCertification().subscribe((data: any) => {
      this.complianceCertList = data;
    });
    this.circuitService.getGeometry().subscribe((data: any) => {
      this.geometryList = data;
    });
    this.circuitService.getOrientation().subscribe((data: any) => {
      this.orientationList = data;
    });

    this.circuitService.getFluidPhase().subscribe((data: any) => {
      this.fluidPhaseList = data;
    });
    this.circuitService.getCorrosivity().subscribe((data: any) => {
      this.corrosivityList = data;
    });
    this.circuitService.getProcessEnvironment().subscribe((data: any) => {
      this.environmentList = data;
    });
    //yesNo
    this.circuitService.getYesNo().subscribe((data: any) => {
      this.toxicMixtureList = data;
      this.flammabilityList = data;
      this.heatTreatmentList = data;
      this.impactTestList = data;
      this.pressureTestList = data;
      this.radiographyList = data;
      this.scheduledRepairList = data;
    });
    this.circuitService.getToxicFluid().subscribe((data: any) => {
      this.toxicFluidList = data;
    });

    this.circuitService.getCriticality().subscribe((data: any) => {
      this.criticalityList = data;
    });
    this.circuitService.getSeismicZoneClassification().subscribe((data: any) => {
      this.seismicZoneList = data;
    });
    this.circuitService.getFireExplosionRisk().subscribe((data: any) => {
      this.fireExplosionRiskList = data;
    });
    this.circuitService.getToxicRisk().subscribe((data: any) => {
      debugger;
      this.toxicRiskList = data;
    });

    this.circuitService.getHeatTreatmentType().subscribe((data: any) => {
      this.heatTreatmentTypeList = data;
    });


    this.circuitService.getPressureTestType().subscribe((data: any) => {
      this.pressureTestTypeList = data;
    });

    this.circuitService.getRadiographyCategory().subscribe((data: any) => {
      this.radiographyCategoryList = data;
    });

    this.circuitService.getGeneralMaterial().subscribe((data: any) => {
      this.generalMaterialList = data;
    });
    this.circuitService.getSchedule().subscribe((data: any) => {
      this.scheduleList = data;
    });
    //YesNoNA
    this.circuitService.getYesNoNa().subscribe((data: any) => {
      debugger
      this.claddingList = data;
      this.liningList = data;
      this.externalCoatingList = data;
      this.insulationList = data;
      this.heatTracingList = data;
      this.fireProofingList = data;
      this.buriedList = data;
      this.cathodicProtectionList = data;
      this.pressureReliefDevicesList = data;
      this.chemicalInjectionList = data;
      this.onlineCorrosionMonitoringList = data;
    });
    this.circuitService.getCladdingType().subscribe((data: any) => {
      this.claddingTypeList = data;
    });
    this.circuitService.getCladdingMaterial().subscribe((data: any) => {
      this.claddingMaterialList = data;
    });

    this.circuitService.getLiningType().subscribe((data: any) => {
      this.liningTypeList = data;
    });
    this.circuitService.getMaterialCertification().subscribe((data: any) => {
      this.materialCertificationList = data;
    });

    this.circuitService.getExternalCoatingType().subscribe((data: any) => {
      this.externalCoatingTypeList = data;
    });

    this.circuitService.getInsulationType().subscribe((data: any) => {
      this.insulationTypeList = data;
    });
    this.circuitService.getInsulationMaterial().subscribe((data: any) => {
      this.insulationMaterialList = data;
    });
    this.circuitService.getCuiPotential().subscribe((data: any) => {
      this.cuiPotentialList = data;
    });
    this.circuitService.getExternalEnvironment().subscribe((data: any) => {
      this.externalEnvironmentList = data;
    });
    this.circuitService.getSupportType().subscribe((data: any) => {
      this.supportTypeList = data;
    });
    this.circuitService.getDetectionSystem().subscribe((data: any) => {
      this.detectionSystemList = data;
    });
    this.circuitService.getIsolationSystem().subscribe((data: any) => {
      this.isolationSystemList = data;
    });
    this.circuitService.getMitigationSystem().subscribe((data: any) => {
      this.mitigationSystemList = data;
    });
    this.circuitService.getCorrosionMonitoringType().subscribe((data: any) => {
      this.corrosionMonitoringTypeList = data;
    });
    this.circuitService.getHazardClassification().subscribe((data: any) => {
      this.hazardClassificationList = data;
    });
    this.circuitService.getCurrentInspectionStrategy().subscribe((data: any) => {
      this.inspectionStrategyList = data;
    });
    this.circuitService.getInspectionAccess().subscribe((data: any) => {
      this.inspectionAccessList = data;
    });


  }

  onFileSelect(event: any) {
    const files = event.target.files;
    if (event.target.files.length > 0) {
      for (let file of event.target.files) {
        this.documentPreviews.push(file);
      }
    }

  }

  downloadFile(file: File) {
    const url = URL.createObjectURL(file);
    const a = document.createElement('a');
    a.href = url;
    a.download = file.name;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  }
  downloadFileExisting(documentId: number, fileName: string) {
    this.circuitService.downloadDocument(documentId).subscribe(blob => {
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = fileName;
      a.click();
      window.URL.revokeObjectURL(url);
    });
  }
  deleteFile(index: number) {
    this.documentPreviews.splice(index, 1);
    this.documentPreviews = [...this.documentPreviews]; // Ensure change detection

    const fileInput = document.getElementById('documents') as HTMLInputElement;
    if (fileInput && this.documentPreviews.length == 0) {
      fileInput.value = '';
    }
  }
  // Helper to update all toggles
  private setAll(state: boolean) {
    this.expand = state;
    this.showGeneral = state;
    this.showLocation = state;
    this.showDesign = state;
    this.showGeometry = state;
    this.showOperation = state;
    this.showFabrication = state;
    this.showMaterial = state;
    this.showExternal = state;
    this.showSafety = state;
    this.showIntegrity = state;
    this.showInspectionSchedule = state;
    this.showMaintenance = state;
    this.showMetadata = state;
    this.showAudit = state;
    this.showDocument = state;
  }
  DeleteCircuit() {
         Swal.fire({
           title: 'Are you sure?You want to delete this circuit!',
           text: 'All data associated with this circuit will be lost',
           icon: 'warning',
           width: '300px',
           showCancelButton: true,
           confirmButtonColor: '#3085d6',
           cancelButtonColor: '#d33',
           confirmButtonText: 'Yes, Delete it!'
         }).then((result) => {
           if (result.isConfirmed) {
             this.circuitService.deleteCircuit(this.childValue).subscribe(
               () => this.backToCircuit(),
               error =>
                 Swal.fire('Delete failed:', error)
             );

           }
         });
       }
}
